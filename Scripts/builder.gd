...
func _process(_delta):
	...
	if Input.is_action_pressed("mine") and break_anim.frame == 0 and tilemap.get_cell_source_id(0,tile_pos) != -1 and raycast.is_colliding():
		...
		var selected_pos
		if not player.godmode: selected_pos = Vector2i(Data.items[selected_index][0],Data.items[selected_index][1])
		else: selected_pos = Vector2i(Data.godmode_items[selected_index][0],Data.godmode_items[selected_index][1])
